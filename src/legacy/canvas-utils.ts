...
export function firstPolygonPoint(
  points: string,
): { x: number; y: number } | null {
...
export function shiftPolygonPoints(
  points: string,
  dx: number,
  dy: number,
): string {
...
export function pathBBoxOnCanvas(
  ...
  const g = canvasEl.querySelector<SVGGElement>(
    `[data-elem-id="${cssAttrEscape(elementId)}"]`,
  );
  if (!g) return null;
  try {
...
export function resolveArrowCoords(
  ...
  if (fromConnected && toConnected) {
    ...
    if (fromAnchor === "auto")
      fromAnchor = pickAutoAnchorTowardPoint(fromEl!, fromState!, toFixed);
    p1 = anchorPointOf(fromEl!, fromState!, fromAnchor);
    p2 = toFixed;
  } else if (fromFixed && toConnected) {
    if (toAnchor === "auto")
      toAnchor = pickAutoAnchorTowardPoint(toEl!, toState!, fromFixed);
    p1 = fromFixed;
    p2 = anchorPointOf(toEl!, toState!, toAnchor);
...
export function resolveLineCoords(
  ...
  const fromFixed =
    typeof l.x1 === "number" && typeof l.y1 === "number"
      ? { x: l.x1, y: l.y1 }
      : null;
  const toFixed =
    typeof l.x2 === "number" && typeof l.y2 === "number"
      ? { x: l.x2, y: l.y2 }
      : null;
