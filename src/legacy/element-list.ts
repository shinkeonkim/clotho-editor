...
function makeDefaultElement(type: string, id: string, cx: number, cy: number): AnimationElement | null {
  switch (type) {
    ...
    case 'image':
      return { type: 'image', id, rotation: 0, appearances: [], tracks: [], x: cx - 50, y: cy - 50, width: 100, height: 100, assetId: registerExternalAsset(placeholderImageUrl()), preserveAspectRatio: 'xMidYMid meet', opacity: 1 };
    case 'path':
      return {
