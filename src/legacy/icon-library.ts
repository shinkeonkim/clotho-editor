...
export class IconLibraryDialog {
  ...
  private bind(): void {
    ...
    this.dialog.addEventListener("click", (e) => {
      ...
      if (catBtn) {
        this.categoryFilter = (catBtn.dataset.iconCat ?? "all") as
          IconCategory | "all";
        this.render();
        return;
      }
  ...
  private filtered(): IconEntry[] {
    const list =
      this.categoryFilter === "all"
        ? ICON_LIBRARY
        : (ICON_GROUPS.find((g) => g.category === this.categoryFilter)
            ?.entries ?? []);
  ...
  private render(): void {
    ...
    const cats: ("all" | IconCategory)[] = [
      "all",
      ...ICON_GROUPS.map((g) => g.category),
    ];
    const catBar = cats
      .map((c) => {
        ...
        const count =
          c === "all"
            ? ICON_LIBRARY.length
            : (ICON_GROUPS.find((g) => g.category === c)?.entries.length ?? 0);
    ...
    if (sent) {
      this.observer = new IntersectionObserver(
        ...
        {
          root: this.dialog.querySelector(".studio-dialog-body") ?? null,
          rootMargin: "200px",
        },
      );
  ...
  private insertIcon(iconId: string): void {
    ...
    const el: ImageElement = {
      ...
      width: 64,
      height: 64,
      assetId: registerExternalAsset(icon.src),
