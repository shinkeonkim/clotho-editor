...
export function selectAdjacentElement(direction: 1 | -1): boolean {
  ...
  if (sel.kind !== "element") {
    const target =
      direction === 1 ? def.elements[0] : def.elements[def.elements.length - 1];
...
export function getVisibleElementIds(
  ...
  return def.elements
    .filter(
      (el) =>
        el.appearances.length === 0 || activeAppearance(el, time) !== null,
    )
    .map((el) => el.id);
}
...
export function handleSelectAll(e: KeyboardEvent): void {
  ...
  const visibleIds =
    time > 0
      ? getVisibleElementIds(def, time)
      : def.elements.map((el) => el.id);
