...
function selectedElementsWithBbox(): Array<{
  el: AnimationElement;
  bbox: Bbox;
}> {
  const def = getDef();
...
export function distributeSelected(kind: DistributeKind): boolean {
  ...
  if (kind === "horizontal") {
    ...
    const lastCenter =
      items[items.length - 1].bbox.x + items[items.length - 1].bbox.w / 2;
    ...
    const lastCenter =
      items[items.length - 1].bbox.y + items[items.length - 1].bbox.h / 2;
    const step = (lastCenter - firstCenter) / (items.length - 1);
    for (let i = 1; i < items.length - 1; i += 1) {
      const targetCenter = firstCenter + step * i;
