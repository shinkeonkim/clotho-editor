...
/**
 * The outermost group containing `elementId`, or null when it is at the root.
 *
 * clotho v1 groups by `parentId` on the child rather than `group.childIds`. The old
 * model could not work: a reference list produces no nested `<g>`, so a group's
 * transform never reached its children — and the legacy renderer had no `group` branch
 * at all. Grouping does something for the first time here.
 */
...
export function ungroupElement(groupId: string): string[] {
  ...
  // Clear the pointers before removing the group, or the children would be re-rooted
  // by the tree builder and reported as having a missing parent.
