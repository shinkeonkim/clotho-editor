import { addElement, getDef, registerDataUriAsset, uniqueElementId } from './state';
...
export async function uploadAndInsertImage(file: File, host: ImageUploadHost): Promise<void> {
  ...
  try {
    ...
    addElement({
      ...
      width: w, height: h, assetId: registerDataUriAsset(dataUrl),
      preserveAspectRatio: 'xMidYMid meet', opacity: 1,
    });
    host.setStatus(`이미지 삽입 완료 (${file.name})`, 'ok');
  } catch (err) {
    host.setStatus('이미지 삽입 실패: ' + (err instanceof Error ? err.message : String(err)), 'error');
  }
}
