...
export async function openLibrary(ui: StudioUi): Promise<void> {
  ...
  if (typeof ui.libraryDialog.showModal === "function")
  ...
  try {
    ...
    if (items.length === 0) {
      ui.libraryList.innerHTML =
        '<li class="studio-library-empty">저장된 애니메이션이 없습니다.</li>';
      return;
    }
...
export async function loadAnimation(ui: StudioUi, id: string): Promise<void> {
  try {
    ...
    setStatus(
      ui,
      "로드 실패: " + (err instanceof Error ? err.message : String(err)),
      "error",
    );
  }
}
...
async function saveDraftWithId(
  ui: StudioUi,
  id: string,
  title: string,
): Promise<void> {
...
export async function deleteCurrent(ui: StudioUi): Promise<void> {
  const def = getDef();
  if (!def) return;
  if (!confirm(`"${def.id}" 을(를) 삭제하시겠습니까? 되돌릴 수 없습니다.`))
    return;
  try {
