...
import { animationDocumentSchema, parseDocument, resolveAsset } from '@shinkeonkim/clotho';
import {
  getDef,
...
beforeEach(() => {
  setDef(
    animationDocumentSchema.parse({ clothoVersion: 1, id: 'demo', duration: 1000, elements: [] }),
  );
});
...
describe('external assets', () => {
  it('registers a URL and returns an id the element can reference', () => {
    ...
    expect(getDef()!.assets[id]).toEqual({ kind: 'external', url: '/uploads/logo.png' });
  });
...
describe('inline assets', () => {
  it('stores raw bytes as base64 so the document is self-contained', () => {
    ...
    expect(getDef()!.assets[id]).toEqual({ kind: 'inline', mime: 'image/png', data: 'AQID' });
  });
  ...
  it('stores a data URI inline rather than as an external reference', () => {
    ...
    expect(getDef()!.assets[id]).toMatchObject({ kind: 'inline', mime: 'image/png' });
  });
