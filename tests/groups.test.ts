...
import { childIdsOf, groupElements, isGroup, ungroupElement } from '../src/legacy/studio-groups';
...
describe('grouping', () => {
  ...
  it('leaves member coordinates untouched', () => {
    groupElements(['a', 'b']);
    const def = getDef()!;
    expect(def.elements.find((el) => el.id === 'a')).toMatchObject({ x: 10, y: 20 });
    expect(def.elements.find((el) => el.id === 'b')).toMatchObject({ x: 100, y: 120 });
  });
